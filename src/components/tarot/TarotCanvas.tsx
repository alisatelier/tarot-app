...
import { CARD_BACK, CARDS } from "@/lib/store/cards";
import { mulberry32, hashSeed } from "@/lib/store/rng";
import { saveReading } from "../../lib/store/persistence";
...
export default function TarotCanvas() {
  ...
  const textures = useMemo(() => {
    const dict: Record<string, PIXI.Texture> = {};
    for (const c of CARDS) dict[c.id] = PIXI.Texture.from(c.frontSrc);
    dict["__back"] = PIXI.Texture.from(CARD_BACK);
    return dict;
  }, []);
  ...
  (async () => {
    ...
    await app.init(); // Pixi v7 async init
    if (destroyed) return;
    ...
    app.ticker.add(() => {
      for (const s of spritesRef.current) {
        ...
        s.front.rotation = s.reversed ? angle + Math.PI : angle;
        s.back.rotation  = s.reversed ? angle + Math.PI : angle;
  ...
  async function dealToSpread() {
    ...
    // Clear prior
    for (const s of spritesRef.current) {
      PIXI.Sprite.removeFrom(s.front);
      PIXI.Sprite.removeFrom(s.back);
    ...
    setAssignments(picks.map((p, i) => ({ slotKey: spread.slots[i].key, cardId: p.id, reversed: p.reversed })));
    ...
    const makeCard = (cardId: string, reversed: boolean, slotKey: string) => {
      ...
      const front = new PIXI.Sprite(PIXI.Texture.from(CARDS.find(c => c.id === cardId)!.frontSrc));
      const back = new PIXI.Sprite(PIXI.Texture.from(CARD_BACK));
      ...
      const entity: SpriteEntity = { body, front, back, isFaceUp: false, reversed, slotKey };
    ...
    const W = app.renderer.width;
    const H = app.renderer.height;
    const toXY = (xPerc: number, yPerc: number) => ({ x: (xPerc / 100) * W, y: (yPerc / 100) * H });

    for (let i = 0; i < spread.slots.length; i++) {
      const slot = spread.slots[i];
      const { x, y } = toXY(slot.xPerc, slot.yPerc);
      const card = makeCard(assignments[i]?.cardId ?? picks[i].id, assignments[i]?.reversed ?? picks[i].reversed, slot.key);
  ...
  function saveCurrentReading() {
    if (!seed) return;
    const result = spritesRef.current.map((s) => ({
      id: CARDS.find(c => c.frontSrc === s.front.texture.baseTexture.resource?.url)?.id || "",
      reversed: s.reversed,
      slotKey: s.slotKey,
    }));
    saveReading({
  ...
  return (
    ...
    {/* ===== 2) SPREAD OPTIONS + ACTIONS JUST BELOW ===== */}
    <div className="flex flex-wrap items-center gap-3 mb-4">
      ...
      {seed && (
        <span className="text-sm text-neutral-500">
          Shareable: add <code>?seed={seed}</code> to this page URL
        </span>
      )}
    </div>

    {/* Extra fields for Focus/Choice spread (still part of “options”) */}
    {isFocusChoiceSpread && (
